    ...
    <>
      {liveKitUrl && (
        ...
        >
          ...
          />
          <RoomContent />
          {/* Any additional components or logic that should be rendered inside LiveKitRoom */}
        </LiveKitRoom>
      )}
    </>
  );
...
const RoomContent = () => {
  ...
  console.log('Checkagent', agentVideoTrack);
  // You can now use participants and tracks to manage the state or render UI components.

  return (
    <div>
      {agentVideoTrack ? (
        <VideoTrack trackRef={agentVideoTrack} className="video-track-class" />
      ) : (
        <p>No agent video track available</p>
      )}
    </div>
  );
};
